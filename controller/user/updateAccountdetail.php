<?php
require '../../model/user/update_model.php';
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($data['phone'] && $data['pass'] && $data['username'] && $data['mail']){
        ...
        if($validFormat){
            $result = check_Update($phone, $pass, $mail, $username); 
            if (!$result){
                http_response_code(401);
                echo json_encode(['Update fail' => $pass]);
            }
            else{
                http_response_code(200);
                echo json_encode(['Update successful'=> 'Update succesfull.']);
            }
        }
        else{
